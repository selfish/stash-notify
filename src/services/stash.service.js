...
app.factory('stash', ['$localStorage', 'util', 'badge', ($ls, util, badge) => {
    ...
    function prFetch() {
        ...
        return util.get(urls.pullRequestsURL)
            ...
            .then((res) => {
                if (res.size == 0) badge.clear();
                else badge.set(res.size, null, "#ff0000");
                $ls.prData = res;
                _log('--- Success.');
                $ls.error = false;
                return res;
    ...
    function prFetchMine() {
        ...
        util.get(urls.myPullRequestsURL)
            .then(_prPrepare)
            .then(_getTasks)
            .then(_getMergeStatus)
            .then((res) => {
...
// previously filterResult
    function _prPrepare(data) {
        ...
        // Open tasks:
        if ($ls.config.hide_pr_with_tasks) {
    ...
    function _me() {
        if (!_.isUndefined($ls.me)) return $ls.me;
        else {
            // Will return null, and skip the check.
            util.get(urls.whoAmI)
                .then((data) => {
                    $ls.me = data;
                    _log(`me fetched: '${$ls.me}'`);
                }).catch(_.noop);
        }
    }
